...
let fullName = [runners.forEach(value => console.log(value.first_name + " " + value.last_name))]
    // console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each first name into all caps and log the result
let allCaps = [runners.map(function (value){
...
let reducer = (accu, cur) => {return accu + cur};
let ticketPriceTotal = donations.reduce(reducer);
console.log(JSON.stringify(ticketPriceTotal));
